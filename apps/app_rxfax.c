...
CALLWEAVER_FILE_VERSION("$HeadURL: https://svn.callweaver.org/callweaver/branches/rel/1.2/apps/app_rxfax.c $", "$Revision: 4723 $")
...
static int rxfax_t38(struct cw_channel *chan, t38_terminal_state_t *t38, char *file, int calling_party,int verbose, int ecm) {
    ...
    if (ecm) {
        ...
        cw_log(LOG_DEBUG, "Enabling ECM mode for app_rxfax\n"  );
    }
    else 
    {
...
static int rxfax_exec(struct cw_channel *chan, int argc, char **argv)
{
    fax_state_t 	fax;
...
int unload_module(void)
{
    ...
    res |= cw_unregister_application(rxfax_app);
    return res;
}
/*- End of function --------------------------------------------------------*/
...
{
    rxfax_app = cw_register_application(rxfax_name, rxfax_exec, rxfax_synopsis, rxfax_syntax, rxfax_descrip);
    return 0;
}
/*- End of function --------------------------------------------------------*/
